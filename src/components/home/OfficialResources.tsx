...
import { OFFICIAL_RESOURCES, PUBLISHER } from "@/lib/constants";
import { GlassCard } from "@/components/ui/GlassCard";
import { SectionHeading } from "@/components/ui/SectionHeading";
import { ExternalLink } from "@/components/ui/ExternalLink";

export function OfficialResources() {
  const t = useTranslations("home.officialResources");
    ...
    <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6">
      ...
      <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {OFFICIAL_RESOURCES.map((resource, i) => (
          ...
          >
            <ExternalLink href={resource.href} className="group block h-full">
              <GlassCard hover className="flex h-full flex-col">
                ...
                <span className="mt-2 text-xs text-gray-500">{PUBLISHER.name}</span>
              </GlassCard>
            </ExternalLink>
          </motion.div>
        ))}
